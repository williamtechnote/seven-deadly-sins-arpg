...
class HelpScene extends Phaser.Scene {
    ...
    create() {
        ...
        const sections = [
            ...
            { title: '交互/界面', items: ['F — NPC / 事件房交互', '事件房祭坛靠近提示也会按 Phaser 文本实际宽度贴在当前视口内，因此贴近屏幕边缘时不会被裁出画面', '右侧固定侧栏里的章节标题、区域名、本局词缀、本局挑战与事件房摘要会优先按 Phaser 文本实际宽度钳制，并按实际文本高度动态纵向排布，避免长标题 / 长路线结算继续互相顶出 HUD', '这些 compact / ultra-compact / ultra-tight 分档会按实际显示尺寸触发，而不再只依赖固定逻辑画布尺寸', 'regular 三行挑战摘要的正文行若遇到上游已带“本局挑战：”/“挑战：”前缀的标签，也会先去重，避免与标题行重复同一前缀；若上游标签重复混入“本局”/“挑战：”这类前缀，也会继续循环去重直到收敛成真正的目标文案；若去重后已无剩余正文，则 regular / compact 摘要会统一回退为“未知挑战”；当 regular 第三行宽度预算继续吃紧时，进行中与完成态也会先沿用“进度:12/30  奖励:+90金 -> 进度:12/30 -> 12/30”/“进度:30/30  奖励:+90金 -> 进度:30/30 -> 30/30”这条语义回退链，而不是直接退化成通用省略；若当前 challenge 没有奖励短句，则 regular 第三行会继续沿用“进度:12/30 -> 12/30”/“进度:30/30 -> 30/30”，不插入额外奖励占位；若 regular 第三行的奖励短句未来扩展到“+9999金 +净化”这类复合形式，进行中 / 完成态也都会继续沿用同一条进度优先回退链', '若视口进入 compact 档位，则本局词缀与事件房摘要会额外收敛为有限行数，并在最后一行补省略号', 'compact 双行挑战摘要里的进行中与完成态都会在第二行继续显式暴露共享奖励短句；若上游挑战标题仍带“本局挑战：”/“挑战：”前缀，compact 第二行也会先去重再拼接奖励短句，避免紧凑摘要重复“挑战”标题；当 compact 进行中摘要的第二行宽度预算继续吃紧时，也会先沿用“击败 30 个敌人 · +90金 -> 击败 30 个敌人 -> 击败30个敌人”这条语义回退链，而不是直接退化成通用省略；完成态的第二行宽度预算继续吃紧时，也会沿用同一条“击败 30 个敌人 · +90金 -> 击败 30 个敌人 -> 击败30个敌人”语义回退链；若当前 challenge 没有奖励短句，则 compact 第二行会继续沿用“击败 30 个敌人 -> 击败30个敌人”，不补额外占位；若这条 compact 第二行的奖励短句未来扩展到“+9999金 +净化”这类复合形式，进行中 / 完成态也都会继续沿用同一条回退链', '若视口进一步进入 ultra-compact 档位，则会先进一步收紧各区块间距与底边缓冲，本局词缀会压到 1 行、事件房摘要压到 2 行、本局挑战压到单行进度摘要；当这条可见摘要的宽度预算继续吃紧时，即使奖励数值扩大到“+9999金”这类长度，进行中态也会继续沿用“挑战 12/30 · +90金 -> 挑战 12/30 -> 12/30”这条语义回退链，完成态则继续沿用“挑战完成 · +90金 -> 挑战完成 -> 完成”，而不会额外插入新的中间短句；若这条可见摘要的奖励短句未来扩展到“+9999金 +净化”这类复合形式，也会继续沿用同一条可见摘要与完成徽记回退链；即使上游挑战标签在 regular / compact 路径里因前缀去重而回退成“未知挑战”，ultra-compact 这条单行摘要也仍会保持同一组“挑战 12/30 · +90金 -> 挑战 12/30 -> 12/30”/“挑战完成 · +90金 -> 挑战完成 -> 完成”语义短句，不额外插入“未知挑战”这类中间短句；regular / compact 分档里凡是仍会显示奖励的路径，也会复用同一奖励短句 helper，避免与 ultra-compact 回退链出现文案漂移；共享 challenge 标签与显式奖励短句 helper 也会压缩异常半角 / 全角空白，避免正文间距或复合奖励文案因脏输入而提前挤爆各分档宽度预算；若该挑战摘要与本局词缀正文都因溢出被隐藏，则会在挑战起步后把“进12/30”/“完成”压成挂在“本局词缀”标题后的轻量徽记；若标题预算进一步吃紧，则进行中态还会继续压成“12/30”；若进入 ultra-tight 更紧预算，则会再回退为“进12”这类无省略最终短句；若连进行中态的“进12”都放不下，则也会静默隐藏 badge，把同一行预算完全还给标题；完成态还会先从“完成+90金”这类奖励短句回退为“完成”；若连完成态的“完成”都放不下，则会静默隐藏 badge，把同一行预算完全还给标题', '该轻量徽记会拆成独立弱化色阶，并进一步下调字级与透明度后再与“本局词缀”标题分开贴边；若标题预算继续压窄，则会按更紧预算分档继续下调 badge 宽度占比、最小宽度与固定 gap，优先把更多横向空间留给标题正文', '若侧栏总高度仍超出安全范围，则会优先隐藏事件房摘要，其次再隐藏本局词缀正文，最后才隐藏本局挑战摘要', 'Tab — 背包', 'Esc — 暂停', 'H — 操作指引'] }
        ];
