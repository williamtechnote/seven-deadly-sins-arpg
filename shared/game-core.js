(function initGameCore(root, factory) {
    ...
    function getBossTargetCueByKey(bossKey) {
        const safeBossKey = typeof bossKey === 'string' ? bossKey.trim() : '';
        return safeBossKey && HUB_PORTAL_TARGET_CUES[safeBossKey]
            ? HUB_PORTAL_TARGET_CUES[safeBossKey]
            : '';
    }
    ...
    function normalizeHubPortalTarget(target) {
        ...
        return {
            label,
            bossKey,
            bossCue: getBossTargetCueByKey(bossKey)
        };
    }
    ...
    function buildRunEventRoomChoiceFooter(choices, state) {
        const recommendation = buildRunEventRoomChoiceRecommendation(choices, state);
        if (recommendation) return recommendation;

        const safeState = state && typeof state === 'object' ? state : {};
        const bossCue = getBossTargetCueByKey(safeState.bossKey);
        return bossCue ? `当前目标：${bossCue}` : '';
    }
