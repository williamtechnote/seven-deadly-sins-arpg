# Challenge Presentation/Ornamental Mixed Decorators Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Explicitly support nested square+presentation-form and nested square+ornamental double-prime challenge decorators across regression checks, README/help text, and TODO bookkeeping while leaving the ornamental low double-prime follow-up active.
...
**Files:**
- Modify: `TODO.md`
- Create: `docs/plans/2026-03-11-challenge-presentation-ornamental-mixed-design.md`
- Create: `docs/plans/2026-03-11-challenge-presentation-ornamental-mixed-plan.md`

**Step 1: Record the next three mixed-wrapper candidates**

- Keep `nested square/presentation-form mixed decorator wrappers` first.
- Promote `nested square/ornamental double-prime mixed decorator wrappers` as the second active item.
- Leave `nested square/ornamental low double-prime mixed decorator wrappers` queued as the next follow-up.
...
Expected: syntax checks pass, regression checks fail on the new nested wrapper contract assertions while docs are still missing the new examples.
...
- Leave the ornamental low double-prime mixed wrapper item active.

**Step 2: Delivery bookkeeping**

- Attempt `git switch -c feat/auto-challenge-presentation-ornamental-mixed`.
- If local branch creation is blocked by git lock permissions, keep working on the current branch, push `HEAD` to `origin/feat/auto-challenge-presentation-ornamental-mixed`, and record the blocker in `PROGRESS.log`.
