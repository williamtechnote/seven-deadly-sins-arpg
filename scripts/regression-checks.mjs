...
function testLustPhase3CadenceArtifactBundle() {
    ...
    assert.deepEqual(
        ...
        'lust cadence artifact bundle should preserve the live telegraph snapshot including counterWindowMs for artifact export'
    );
    assert.deepEqual(
...
function testReadmeLustCadenceReportChecklist() {
    ...
    assert.match(
        source,
        /`review checkpoint #n` 索引短注记、bridge\/loopback checkpoint alias short note、`dash\/charmBolt` bridge count short note、`bridgeTimeline` index short note、`counterWindowMs` short note、`counterWindowRatio` short note、`counterWindowDeltaMs` short note、`counterWindowTailOffsetMs` short note、`counterWindowTailPhase` short note、`counterWindowClosureCue` short note 与 `telegraphDurationMs` short note/,
        'README should document the counterWindowMs, counterWindowRatio, counterWindowDeltaMs, counterWindowTailOffsetMs, counterWindowTailPhase, counterWindowClosureCue and telegraphDurationMs short notes in the drift-only mini checklist'
    );
}
