...
function testE2eReportPhase3CadenceMarkdownIndex() {
    ...
    assert.match(
        output,
        /- Phase 3 汇总: match=2 \| drift=0 \| current recovery checkpoint: `review checkpoint #1 reverseControl->illusion`/,
        'e2e report should surface a phase-3 summary header with the live recovery checkpoint inline'
    );
    assert.doesNotMatch(
        output,
...
function testReadmeLustCadenceReportChecklist() {
    ...
    assert.match(
        source,
        /`current recovery checkpoint` 直接编进 report 的 recovery 短句与 summary/,
        'README should document that the phase-3 report now inlines the current recovery checkpoint in both the recovery note and summary'
    );
    assert.match(
        source,
