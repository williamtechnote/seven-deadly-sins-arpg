...
function testLustPhase3AttackOrder() {
    ...
    assert.deepEqual(
        ...
        ['dash', 'charmBolt', 'dash', 'charmBolt', 'dash', 'charmBolt', 'dash', 'charmBolt', 'dash', 'charmBolt', 'dash', 'charmBolt', 'dash', 'charmBolt', 'dash', 'charmBolt', 'dash', 'charmBolt'],
        'lust phase 3 should keep the loopback after mirageDance on an even longer directed light-pressure bridge again after the shared recovery follow-up'
    );
}
...
function testLustMirageLoopbackBridgeFollowup() {
    ...
    assert.equal(
        ...
        18,
        'lust phase 3 should add yet another dash-charmBolt loopback step before returning to reverseControl'
    );
}
...
function testLustSharedMajorRecoveryHooks() {
    ...
    assert.equal(
        ...
        6800,
        'lust phase 3 should stretch the shared recovery window for major specials again after the eight-breather lock and longer mirageDance loopback bridge'
    );
    assert.deepEqual(
...
function testReadmeLustSharedMajorRecovery() {
    ...
    assert.match(
        source,
        /共享 `majorSpecial` recovery window 这轮也会在八码轻压守卫与更长的 `mirageDance` -> `reverseControl` loopback 过桥都落地后继续再拉长一档，让下一段 `reverseControl` \/ `illusion` \/ `mirageDance` 再继续晚半拍回切/,
        'README should document the newest shared major-special recovery extension after the eight-breather lock and longer mirageDance loopback bridge'
    );
    assert.match(
        source,
    ...
    assert.match(
        source,
        /并再多压一组 `dash` \/ `charmBolt`/,
        'README should document the extra dash-charmBolt pair added to the mirageDance loopback bridge'
    );
}
