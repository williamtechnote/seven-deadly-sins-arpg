...
function testReadmeKeyboardInventoryLoop() {
    ...
    assert.match(
        source,
        /回到 Hub 后，画面上还会保留一个小型 `上轮战报`，至少会把 `已讨伐谁 \/ 哪条路线收官 \/ 源于哪次抉择` 继续钉在下一次选门前[\s\S]*?当前 hub route-memory baseline 已明确收束为这组 `上轮战报 \+ 选门参考` 双卡片，现阶段不再额外新增 run-history 入口；后续 prep surface 已交由 `备战参考 \/ 采购参考 \/ 备战复查` 各自承接。即使还没有 `上轮战报`/,
