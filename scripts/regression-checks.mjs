...
function testReadmeKeyboardInventoryLoop() {
    ...
    assert.match(
        source,
        /回到 Hub 后，画面上还会保留一个小型 `上轮战报`，至少会把 `已讨伐谁 \/ 哪条路线收官 \/ 源于哪次抉择` 继续钉在下一次选门前[\s\S]*?当前 hub route-memory baseline 已收束为这组 `上轮战报 \+ 选门参考` 双卡片/,
    ...
    assert.match(
        source,
        /首段普通战斗清场后，穿过首段 corridor 时还会再补一次 `承接 稳拍反制` \/ `承接 回体扛压` 这类短 cue[\s\S]*?当前这条 early-run ladder 现已稳定成“首战开压 \+ corridor handoff \+ shrine 靠近”的轻量三段接力/,
...
function testHelpOverlayQuickSlotLoop() {
    ...
    assert.match(
        source,
        /Hub 里的“上轮战报 \+ 选门参考”若已经承接住上轮收官与当前门前姿态，真正踏进关卡后的第一秒还会再补一次“目标 傲慢 · 稳线读招”\/“目标 暴怒 · 回体扛压”\/“目标 色欲 · 稳拍反制”这类一次性开局提示/,
    ...
    assert.match(
        source,
        /若首段普通战斗已经清场，但首个未结算 shrine 还没贴近，穿过首段 corridor 时也会再补一次“承接 稳拍反制”\/“承接 回体扛压”这类短 cue，把目标姿态继续保到第一次路线抉择前[\s\S]*?目前这段 early-run handoff 已稳定形成“首战开压 \+ corridor handoff \+ shrine 靠近”的三段接力/,
