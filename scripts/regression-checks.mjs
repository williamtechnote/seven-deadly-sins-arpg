...
function testStatusRoutingRunEffectHooks() {
    ...
    assert.match(
        readme,
        /若已选 `镇压圣坛` 的 `镇步修习`，`特攻 O` 在持有附带减速的武器时会显示 `镇步\+45%`，切到其他武器则改成 `镇步切减速`，而当强化后的减速真正命中挂上时还会补一个 `镇步` 提示；若已选 `破势修习`，`特攻 O` 在持有可附带减速的武器时会显示 `破势\+28%`，切到其他武器则改成 `破势切减速`，而当这段对减速目标的加伤真正命中兑现时会先短促切成 `破势命中`，同时继续保留命中处的 `破势` 浮字/,
