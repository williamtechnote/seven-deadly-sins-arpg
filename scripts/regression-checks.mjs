...
function testReadmeLustCadenceReportChecklist() {
    ...
    assert.match(
        source,
        /`review checkpoint #n` 索引短注记、bridge\/loopback checkpoint alias short note、`dash\/charmBolt` bridge count short note、`bridgeTimeline` index short note、`counterWindowMs` short note、`counterWindowRatio` short note 与 `telegraphDurationMs` short note/,
        'README should document the counterWindowMs, counterWindowRatio and telegraphDurationMs short notes in the drift-only mini checklist'
    );
}
