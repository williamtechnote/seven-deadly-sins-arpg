...
function testRunEventEncounterRoutingHooks() {
    ...
    assert.match(
        source,
        /buildRunEventRoomPromptLabel\(eventRoom,\s*RUN_EVENT_ROOM_POOL,\s*\{\s*bossKey:\s*this\.bossKey\s*\}\)/,
        'level-scene shrine prompts should thread the current boss target into the shared prompt helper so the first approach label can keep the portal posture alive'
    );
    assert.match(
        source,
