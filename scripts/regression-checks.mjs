...
function testRunEventRoomChoiceFooterHelper() {
    assert.equal(typeof buildRunEventRoomChoiceFooter, 'function', 'run-event choice footer helper should be exported');

    const prayerChoices = getRunEventRoomChoices('prayerShrine');
    assert.equal(
        buildRunEventRoomChoiceFooter(prayerChoices, {
            playerHp: 100,
            playerMaxHp: 120,
            bossKey: 'lust',
        ...
        '当前目标：稳拍反制',
        'run-event choice footer helper should fall back to the current boss posture when the first shrine has no stronger recommendation'
    );
    ...
    assert.equal(
        ...
        '建议 2：迅击祷言 · 当前局已偏节奏',
        'run-event choice footer helper should keep the higher-confidence recommendation when one exists'
    );
    ...
    assert.equal(
        ...
        '',
        'run-event choice footer helper should stay silent when there is neither a recommendation nor a boss-aware target'
    );
}
...
function testRunEventEncounterRoutingHooks() {
    ...
    assert.match(
        source,
        /_openRunEventChoicePanel\(\)\s*{[\s\S]*?const previewState = this\._buildRunEventChoicePreviewState\(\);[\s\S]*?const footerMessage = buildRunEventRoomChoiceFooter\(this\._runEventChoiceOptions,\s*previewState\);[\s\S]*?this\._setRunEventChoicePanelFooter\(footerMessage \|\| RUN_EVENT_CHOICE_PANEL_FOOTER_DEFAULT,\s*'default'\);/,
        'run-event choice panel should route the shared preview state into a shared footer helper that prefers recommendations before falling back to the neutral instruction footer'
    );
    assert.match(
        source,
...
function testReadmeKeyboardInventoryLoop() {
    ...
    assert.match(
        source,
        /首个未结算事件房的靠近提示 \/ 世界标签还会继续补 `按F效果 · 稳拍反制` \/ `祈愿圣坛 · 目标 稳拍反制` 这类短 reminder/,
        'README should document the first-shrine posture reminder that keeps the boss target alive into the first route decision'
    );
    assert.match(
        source,
        /若首个事件房 choice panel 暂时还没有更强的 shared recommendation，footer 也会继续补 `当前目标：稳拍反制` \/ `当前目标：回体扛压` 这类中性目标脚注/,
        'README should document the neutral first-shrine target footer that fills the gap when no stronger recommendation is available'
    );
    assert.match(
        source,
        /Tab.*背包/,
...
function testHelpOverlayQuickSlotLoop() {
    ...
    assert.match(
        source,
        /当玩家真正贴近首个未结算事件房时，靠近提示\/世界标签也会继续补“按F效果 · 稳拍反制”\/“祈愿圣坛 · 目标 稳拍反制”这类短 reminder/,
    ...
    assert.match(
        source,
        /若首个事件房 choice panel 暂时还没有更强的 shared recommendation，footer 也会继续补“当前目标：稳拍反制”\/“当前目标：回体扛压”这类中性目标脚注/,
        'help overlay should document the neutral first-shrine target footer that persists boss posture into the actual choice panel'
    );
    assert.match(
        source,
...
function main() {
    ...
    runTest('run-event choice footer helper', testRunEventRoomChoiceFooterHelper);
